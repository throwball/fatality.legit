...
void c_nade_prediction::predict(c_usercmd * user_cmd) noexcept {
	...
	c_math::get().angle_vectors(thrown, thrown_direction);
...
// timers of nades exploding
	...
	case weapon_inc:
		...
		//	decoy and smoke grenades are buggy in prediction so i made work arround heeh
	case weapon_decoy:
	case weapon_smoke:
		if (time > 2.5f) {
			return true;
...
bool is_grenade(void* weapon) {
	...
	static const std::vector<int> v = { WEAPON_FRAG_GRENADE, weapon_he, weapon_inc, weapon_smoke, WEAPON_TAGRENADE, weapon_molotov , weapon_decoy };
...
void c_nade_prediction::draw() noexcept {
	...
	auto draw_3d_dotted_circle = [](vec3_t position, float points, float radius) {
		...
		for (float a = 0; a < 3.141592654f * 2.0f; a += step) {
			...
			if (interfaces::debug_overlay->world_to_screen(start, start2d))
				render::line(start2d.x, start2d.y, start2d.x + 1, start2d.y + 1, color(255, 255, 255));
		}
	};
	...
	if (_predicted) {
		for (auto &p : _points) {
			...
			if (c_math::get().world_to_screen(p.m_start, start) && c_math::get().world_to_screen(p.m_end, end)) {

				render::line(start.x, start.y, end.x, end.y, color(10, 255, 10));
