...
bool zgui::tab_button(const char* id, const vec2 size, bool value) noexcept
{
	...
	int text_wide, text_tall;
	functions.get_text_size(render::main_font, id_split[0].c_str(), text_wide, text_tall);

	context.window.render.emplace_back(zgui_control_render_t{ {draw_pos.x + size.x / 2 - text_wide / 2, draw_pos.y + size.y / 2 - text_tall / 2 }, zgui_render_type::zgui_text, global_colors.color_text,  id_split[0], vec2{0,0}, font });

	if (value)
	{
		context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y + size.y - 2 }, zgui_render_type::zgui_filled_rect, global_colors.color_text , "", {size.x - 1, 1} });
	}


	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x + 1, draw_pos.y + 1 }, zgui_render_type::zgui_filled_rect,hovered ? global_colors.window_border_color : global_colors.window_border_inner_fill , "",{size.x - 2, size.y - 2} });
	context.window.render.emplace_back(zgui_control_render_t{ { draw_pos.x, draw_pos.y }, zgui_render_type::zgui_filled_rect, global_colors.control_outline, "", size });
