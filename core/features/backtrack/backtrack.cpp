...
void c_backtrack::update() noexcept {
	...
	if (!c_system.cfg.backtrack || !local_player || !local_player->is_alive()) {
		if (!records->empty())
			records->clear();
	...
	for (int i = 1; i <= interfaces::globals->max_clients; i++) {
		...
		while (records[i].size() > 3 && records[i].size() > static_cast<size_t>(time_to_ticks(static_cast<float>(c_system.cfg.backtrack_ms) / 1000.f)))
			records[i].pop_back();
...
void c_backtrack::run(c_usercmd* cmd) noexcept {
	if (!c_system.cfg.backtrack || !c_system.cfg.misc_enabled)
		return;
